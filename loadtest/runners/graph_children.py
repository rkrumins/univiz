"""Single-scenario Locust runner: children + children-with-edges only.

Used by ``make stress-children``. Wraps ``GraphChildrenTasks`` in an
...
class GraphChildrenUser(HttpUser):
    host = SETTINGS.host
    wait_time = between(SETTINGS.think_min, SETTINGS.think_max)
    tasks = {GraphChildrenTasks: 1}
    id_pool: IdPool
